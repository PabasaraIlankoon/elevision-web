...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    return () => { unsubAlerts(); unsubDevices(); };
  }, []);
  ...
  const handleAcknowledge = async () => {
    ...
    setAcking(true);
    try { await markAlertSeen(heroAlert.id); }
    ...
    <div className="grid-bg min-h-screen">
      <div className="p-4 md:p-6 space-y-6">
        ...
        {heroAlert ? (
          ...
          <div className="flex flex-col lg:flex-row gap-4" style={{ height: "auto" }}>

            {/* LEFT: image — fixed square, capped */}
            <div className="lg:w-1/3 lg:flex-shrink-0">
              <div
                className={`relative rounded-2xl overflow-hidden border-2 ${
                  isNew ? "border-amber-400/60" : "border-border"
                }`}
              ...
              >
                ...
                <div className="relative w-full h-full">
                  ...
                  <div className="absolute top-3 left-3">
                    {isNew ? (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-400 text-black text-xs font-bold shadow">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-3">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <p className="text-xs text-white/60 font-mono">{heroAlert.deviceId}</p>
            ...
            <div className="flex-1 flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3 flex-1">
                <StatTile
                  ...
                  icon={<AlertCircle className="w-3.5 h-3.5 text-amber-400/60" />}
                  valueClass="text-amber-400"
                />
                <StatTile
                  label="Active Alerts"
                  value={loading ? "…" : String(activeCount)}
                  icon={<TrendingUp className={`w-3.5 h-3.5 ${activeCount > 0 ? "text-amber-400/60" : "text-muted-foreground"}`} />}
                  valueClass={activeCount > 0 ? "text-amber-400" : "text-foreground"}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={AlertCircle} label="Total Detections Today" value={loading ? "…" : todayCount} accent />
            <StatCard icon={TrendingUp} label="Active Alerts" value={loading ? "…" : activeCount} accent={activeCount > 0} />
            <StatCard icon={Radio} label="Devices Online" value={loading ? "…" : `${onlineDevices}/${devices.length || 0}`} />
            <StatCard icon={Clock} label="Last Detection" value={loading ? "…" : lastDetectionTime} />
          </div>
        )}
...
function StatTile({
  ...
  return (
    <div className={`rounded-xl border bg-card p-4 flex flex-col justify-between min-h-0 ${
      highlight ? "border-amber-400/40" : "border-border"
    }`}>
      <div className="flex items-center justify-between gap-1 mb-2">
        <p className="text-sm font-medium text-muted-foreground uppercase tracking-wide leading-tight">{label}</p>
        {icon}
      </div>
      <p className={`font-bold leading-none ${smallValue ? "text-3xl" : "text-4xl"} ${valueClass}`}>
        {value}
      </p>
    </div>
  );
